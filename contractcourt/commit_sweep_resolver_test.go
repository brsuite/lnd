...
func TestCommitSweepResolverNoDelay(t *testing.T) {
	...
	amt := bronutil.Amount(res.SelfOutputSignDesc.Output.Value)
...
func testCommitSweepResolverDelay(t *testing.T, sweepErr error) {
	...
	expectedReport := ContractReport{
		...
		Amount:       bronutil.Amount(amt),
		LimboBalance: bronutil.Amount(amt),
	}
	...
	expectedRecoveredBalance := bronutil.Amount(amt)
	if sweepErr != nil {
