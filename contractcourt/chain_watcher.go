...
type chainWatcherConfig struct {
	...
	// known to us.
	isOurAddr func(bronutil.Address) bool
...
// being dust.
func (c *chainWatcher) toSelfAmount(tx *wire.MsgTx) bronutil.Amount {
	var selfAmt bronutil.Amount
	for _, txOut := range tx.TxOut {
		...
		for _, addr := range addrs {
			if c.cfg.isOurAddr(addr) {
				selfAmt += bronutil.Amount(txOut.Value)
			}
		}
	}
