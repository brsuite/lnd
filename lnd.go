...
func Main(cfg *Config, lisCfg ListenerCfg, implCfg *ImplementationCfg,
	...
	var network string
	switch {
	case cfg.Brocoin.TestNet3 || cfg.Litecoin.TestNet3:
		network = "testnet"

	case cfg.Brocoin.MainNet || cfg.Litecoin.MainNet:
		network = "mainnet"

	case cfg.Brocoin.SimNet || cfg.Litecoin.SimNet:
		network = "simnet"

	case cfg.Brocoin.RegTest || cfg.Litecoin.RegTest:
		network = "regtest"

	case cfg.Brocoin.SigNet:
		network = "signet"
	}
	...
	if cfg.Watchtower.Active {
		...
		wtCfg := &watchtower.Config{
			...
			Net:            cfg.net,
			NewAddress: func() (bronutil.Address, error) {
