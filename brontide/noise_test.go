...
func makeListener() (*Listener, *lnwire.NetAddress, error) {
	...
	localPriv, err := bronec.NewPrivateKey(bronec.S256())
	if err != nil {
		return nil, nil, err
	}
...
func establishTestConnection() (net.Conn, net.Conn, func(), error) {
	...
	// within our test.
	remotePriv, err := bronec.NewPrivateKey(bronec.S256())
	if err != nil {
		return nil, nil, nil, err
	}
...
func TestBolt0008TestVectors(t *testing.T) {
	...
	initiatorPriv, _ := bronec.PrivKeyFromBytes(
		bronec.S256(), initiatorKeyBytes,
	)
	...
	responderPriv, responderPub := bronec.PrivKeyFromBytes(
		bronec.S256(), responderKeyBytes,
	)
	...
	initiatorEphemeral := EphemeralGenerator(func() (*bronec.PrivateKey, error) {
	...
	responderEphemeral := EphemeralGenerator(func() (*bronec.PrivateKey, error) {
