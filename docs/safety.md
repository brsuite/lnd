...
Important to know:
* Deleting the `*.macaroon` files in the `<lnd-dir>/data/chain/brocoin/mainnet/`
...
There is
[a tool being worked on](https://github.com/brsuite/lnd/pull/2373)
...
is the `<lnddir>/data/chain/brocoin/mainnet/channel.backup` file which holds
...
`<lnddir>/data/chain/brocoin/mainnet/channel.backup`
...
Multiple `lnd` nodes can run off of a single `brocoind` instance. There will be
connection/thread/performance limits at some number of `lnd` nodes but in
practice running 2 or 3 `lnd` instances per `brocoind` node didn't show any
problems.
