...
spins up a `brond` backend alongside `lnd`. Check out the documentation at
...
```shell
⛰  docker run --name lnd-testnet myrepository/lnd --brocoin.active --brocoin.testnet --brocoin.node=neutrino --neutrino.connect=faucet.lightning.community
```
...
To choose a specific [branch](https://github.com/brsuite/lnd/branches) or [tag](https://hub.docker.com/r/lightninglabs/lnd/tags?page=1&ordering=last_updated) instead, use the "checkout" build-arg. For example, to build the latest tagged commit:
...
```shell
⛰  docker run --name=lnd-testnet -it myrepository/lnd:latest-tag --brocoin.active --brocoin.testnet --brocoin.node=neutrino --neutrino.connect=faucet.lightning.community
```
