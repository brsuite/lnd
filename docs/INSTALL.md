...
* [Available Backend Operating Modes](#available-backend-operating-modes)
  ...
  * [Using brond](#using-brond)
    ...
    * [Running lnd using the brond backend](#running-lnd-using-the-brond-backend)
...
mainnet.
[Visit the release page on GitHub](https://github.com/brsuite/lnd/releases)
...
```shell
⛰  git clone https://github.com/brsuite/lnd
⛰  cd lnd
...
### Preliminaries for installing from source
  In order to work with [`lnd`](https://github.com/brsuite/lnd), the
...
release](https://github.com/brsuite/lnd/releases). Assuming the name
...
commands:
```shell
⛰  cd $GOPATH/src/github.com/brsuite/lnd
⛰  git pull
...
```
If you want to use `lnd` on testnet, `brond` needs to first fully sync the
...
### Running lnd using the brond backend
...
```shell
⛰   lnd --brocoin.active --brocoin.testnet --debuglevel=debug \
...
backend.  Setup will be described in regards to `brocoind`, but note that `lnd`
...
To configure your brocoind backend for use with lnd, first complete and verify
the following:
...
Here's a sample `brocoin.conf` for use with lnd:
```text
testnet=1
server=1
daemon=1
...
`lnd` with `brocoind` as your backend (as with `brocoind`, you can create an
...
- The auth parameters `rpcuser` and `rpcpass` parameters can typically be
  determined by `lnd` for a `brocoind` instance running under the same user,
...
- If you DO choose to explicitly pass the auth parameters in your `lnd.conf` or
  command line options for `lnd` (`brocoind.rpcuser` and `brocoind.rpcpass` as
  shown in example command above), you must also specify the
  `brocoind.zmqpubrawblock` and `brocoind.zmqpubrawtx` options. Otherwise, `lnd`
...
- You must ensure the same addresses are used for the `brocoind.zmqpubrawblock`
  and `brocoind.zmqpubrawtx` options passed to `lnd` as for the `zmqpubrawblock`
  and `zmqpubrawtx` passed in the `brocoind` options respectively.
- When running lnd and brocoind on the same Windows machine, ensure you use
  ...
  slow inter-process-communication between lnd and the brocoind backend.  If lnd
  ...
  messages like this as lnd consumes blocks from brocoind.
- Don't connect more than two or three instances of `lnd` to `brocoind`. With
  the default `brocoind` settings, having more than one instance of `lnd`, or
...
Github](https://github.com/brsuite/lnd/issues/20).
...
`lnddir/data/chain/brocoin/mainnet`) so that it's possible to use a distinct
...
start both `brond` and `lnd` in the `simnet` mode. Simnet is similar to regtest
...
There are currently two primary ways to run `lnd`: one requires a local `brond`
...
launches, allowing you to simply type `lnd --brocoin.testnet --brocoin.active`
...
Here's a sample `lnd.conf` for `brond` to get you started:
```text
[Application Options]
debuglevel=trace
...
Brocoin chain. `lnd` also supports Litecoin testnet4 (but not both BTC and LTC
at the same time), so when working with Litecoin be sure to set to parameters
for Litecoin accordingly. See a more detailed sample config file available
[here](https://github.com/brsuite/lnd/blob/master/sample-lnd.conf)
