...
func parseTestGraph(useCache bool, path string) (*testGraphInstance, error) {
	...
	privKeyMap := make(map[string]*bronec.PrivateKey)
	...
	for _, node := range g.Nodes {
		...
		if len(privBytes) > 0 {
			key, derivedPub := bronec.PrivKeyFromBytes(
				bronec.S256(), privBytes,
			)
	...
	for _, edge := range g.Edges {
		...
		edgeInfo := channeldb.ChannelEdgeInfo{
			...
			Capacity:     bronutil.Amount(edge.Capacity),
		}
...
func symmetricTestChannel(alias1, alias2 string, capacity bronutil.Amount,
...
func asymmetricTestChannel(alias1, alias2 string, capacity bronutil.Amount,
...
type testGraphInstance struct {
	...
	privKeyMap map[string]*bronec.PrivateKey
...
func createTestGraphFromChannels(useCache bool, testChannels []*testChannel,
	...
	addNodeWithAlias := func(alias string, features *lnwire.FeatureVector) (
		*channeldb.LightningNode, error) {
		...
		privKey, pubKey := bronec.PrivKeyFromBytes(bronec.S256(),
			keyBytes)
...
func runPathFindingWithAdditionalEdges(t *testing.T, useCache bool) {
	...
	dogePubKey, err := bronec.ParsePubKey(dogePubKeyBytes, bronec.S256())
	if err != nil {
...
func runPathInsufficientCapacity(t *testing.T, useCache bool) {
	...
	payAmt := lnwire.NewMSatFromSatoshis(bronutil.SatoshiPerBrocoin)
	_, err = dbFindPath(
		graph.graph, nil, &mockBandwidthHints{},
		noRestrictions, testPathFindingConfig,
...
func testProbabilityRouting(t *testing.T, useCache bool,
	paymentAmt bronutil.Amount, p10, p11, p20, minProbability float64,
	expectedChan uint64) {
