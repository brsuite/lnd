...
func runDeriveSharedKey(t *harnessTest, alice *lntest.HarnessNode) {
	...
	ephemeralPriv, err := bronec.NewPrivateKey(bronec.S256())
	...
	assertECDHMatch := func(pub *bronec.PublicKey,
	...
	nodePub, err := bronec.ParsePubKey(alice.PubKey[:], bronec.S256())
...
func deriveCustomizedKey(ctx context.Context, node *lntest.HarnessNode,
	family, index int32) (*bronec.PublicKey, error) {
	...
	pub, err := bronec.ParsePubKey(resp.RawKeyBytes, bronec.S256())
	if err != nil {
