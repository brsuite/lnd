...
func channelForceClosureTest(net *lntest.NetworkHarness, t *harnessTest,
	...
	const (
		chanAmt     = bronutil.Amount(10e6)
		pushAmt     = bronutil.Amount(5e6)
		paymentAmt  = 100000
		numInvoices = 6
	)
	...
	var totalFeeCarol bronutil.Amount
	for _, tx := range sweepTxns {
		fee, err := getTxFee(net.Miner.Client, tx)
		require.NoError(t.t, err)
	...
	carolExpectedBalance := bronutil.Amount(carolStartingBalance) +
		pushAmt - totalFeeCarol

	// In addition, if this is an anchor-enabled channel, further add the
	// anchor size.
	if channelType == lnrpc.CommitmentType_ANCHORS {
		carolExpectedBalance += bronutil.Amount(anchorSize)
	}

	require.Equal(
		t.t, carolExpectedBalance,
		bronutil.Amount(carolBalResp.ConfirmedBalance),
