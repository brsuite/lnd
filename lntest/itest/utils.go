...
func createPayReqs(node *lntest.HarnessNode, paymentAmt bronutil.Amount,
...
func calcStaticFee(c lnrpc.CommitmentType, numHTLCs int) bronutil.Amount {
	const htlcWeight = input.HTLCWeight
	var (
		feePerKw     = chainfee.SatPerKVByte(50000).FeePerKWeight()
		commitWeight = input.CommitWeight
		anchors      = bronutil.Amount(0)
	)
...
func getTxFee(miner *rpcclient.Client, tx *wire.MsgTx) (bronutil.Amount, error) {
	...
	for _, out := range tx.TxOut {
		balance -= bronutil.Amount(out.Value)
	}
