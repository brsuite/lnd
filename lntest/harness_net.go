...
import (
	...
	"github.com/brsuite/lnd"
	"github.com/brsuite/lnd/kvdb/etcd"
...
func (n *NetworkHarness) SetUp(t *testing.T,
	...
	for _, client := range clients {
		for i := 0; i < 10; i++ {
			...
			addr, err := bronutil.DecodeAddress(resp.Address, n.netParams)
			if err != nil {
				return err
			}
...
func (n *NetworkHarness) OpenPendingChannel(srcNode, destNode *HarnessNode,
	amt bronutil.Amount,
	pushAmt bronutil.Amount) (*lnrpc.PendingUpdate, error) {
...
func (n *NetworkHarness) SendCoinsUnconfirmed(t *testing.T, amt bronutil.Amount,
	target *HarnessNode) {
...
func (n *NetworkHarness) sendCoins(amt bronutil.Amount, target *HarnessNode,
	...
	// and exit.
	if !confirmed {
		expectedBalance := bronutil.Amount(initialBalance.UnconfirmedBalance) + amt
	...
	expectedBalance := bronutil.Amount(fullInitialBalance) + amt
