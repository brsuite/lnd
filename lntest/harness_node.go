...
import (
	...
	"github.com/brsuite/lnd/lnrpc/watchtowerrpc"
	"github.com/brsuite/lnd/lnrpc/wtclientrpc"
...
// closeChanWatchRequest is a request to the brsuiteWatcher to be
...
// brsuiteWatcher is a goroutine which is able to dispatch
...
func (hn *HarnessNode) brsuiteWatcher() {
	defer hn.wg.Done()
...
func (hn *HarnessNode) WaitForBalance(expectedBalance bronutil.Amount,
	confirmed bool) error {

	req := &lnrpc.WalletBalanceRequest{}

	var lastBalance bronutil.Amount
	doesBalanceMatch := func() bool {
		...
		if confirmed {
			lastBalance = bronutil.Amount(balance.ConfirmedBalance)
			return bronutil.Amount(balance.ConfirmedBalance) ==
				expectedBalance
		}

		lastBalance = bronutil.Amount(balance.UnconfirmedBalance)
		return bronutil.Amount(balance.UnconfirmedBalance) ==
			expectedBalance
	}
