...
import (
	...
	"github.com/brsuite/lnd/htlcswitch/hodl"
	...
	"github.com/brsuite/lnd/lntest/wait"
...
func TestChannelLinkRevThenSig(t *testing.T) {
	...
	const chanAmt = bronutil.SatoshiPerBrocoin * 5
	const chanReserve = bronutil.SatoshiPerBrocoin * 1
...
func TestExitNodeTimelockPayloadMismatch(t *testing.T) {
	...
	const amount = bronutil.SatoshiPerBrocoin
...
func TestLinkForwardMaxHTLCPolicyMismatch(t *testing.T) {
	...
	channels, cleanUp, _, err := createClusterChannels(
		bronutil.SatoshiPerBrocoin*5, bronutil.SatoshiPerBrocoin*5,
	)
	if err != nil {
		t.Fatalf("unable to create channel: %v", err)
	}
...
func newSingleLinkTestHarness(chanAmt, chanReserve bronutil.Amount) (
...
func TestChannelLinkBandwidthChanReserve(t *testing.T) {
	...
	const bobChanAmt = bronutil.SatoshiPerBrocoin * 1
	const bobChanReserve = bronutil.SatoshiPerBrocoin * 1.5
...
func TestChannelLinkUpdateCommitFee(t *testing.T) {
	...
	// for this test.
	const aliceInitialBalance = bronutil.SatoshiPerBrocoin * 3
	channels, cleanUp, _, err := createClusterChannels(
		aliceInitialBalance, bronutil.SatoshiPerBrocoin*5,
	)
	if err != nil {
		t.Fatalf("unable to create channel: %v", err)
	}
