...
func TestTxWeightEstimator(t *testing.T) {
	...
	p2pkhAddr, err := bronutil.NewAddressPubKeyHash(
	...
	p2wkhAddr, err := bronutil.NewAddressWitnessPubKeyHash(
		make([]byte, 20), netParams)
	if err != nil {
		t.Fatalf("Failed to generate address: %v", err)
	}
	...
	p2wshAddr, err := bronutil.NewAddressWitnessScriptHash(
	...
	p2shAddr, err := bronutil.NewAddressScriptHash([]byte{0}, netParams)
	if err != nil {
		t.Fatalf("Failed to generate address: %v", err)
	}
	...
	for i, test := range testCases {
		...
		expectedWeight := blockchain.GetTransactionWeight(bronutil.NewTx(tx))
...
func TestTxSizes(t *testing.T) {
	for _, test := range txSizeTests {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			weight := blockchain.GetTransactionWeight(bronutil.NewTx(tx))
