...
func randPubKeyHashScript() ([]byte, *bronec.PrivateKey, error) {
	...
	pubKeyHash := bronutil.Hash160(privKey.PubKey().SerializeCompressed())
	addrScript, err := bronutil.NewAddressPubKeyHash(pubKeyHash, NetParams)
	if err != nil {
		return nil, nil, err
	}
...
// NewMiner spawns testing harness backed by a brond node that can serve as a
// miner.
...
// NewBrocoindBackend spawns a new brocoind node that connects to a miner at the
...
// spawned brocoind node is returned.
func NewBrocoindBackend(t *testing.T, minerAddr string,
	txindex bool) (*chain.BrocoindConn, func()) {
	...
	brocoind := exec.Command("brocoind", args...)
	if err := brocoind.Start(); err != nil {
		os.RemoveAll(tempBrocoindDir)
		t.Fatalf("unable to start brocoind: %v", err)
	}
	...
	conn, err := chain.NewBrocoindConn(&chain.BrocoindConfig{
	...
	if err != nil {
		...
		brocoind.Wait()
		os.RemoveAll(tempBrocoindDir)
		t.Fatalf("unable to establish connection to brocoind: %v", err)
	}
...
func NewNeutrinoBackend(t *testing.T, minerAddr string) (*neutrino.ChainService, func()) {
	...
	// Create an instance of neutrino connected to the running brond
	// instance.
	...
	// generated by the brond instance.
	spvNode.Start()
