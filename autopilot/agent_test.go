...
func (m *mockConstraints) ChannelBudget(chans []LocalChannel,
	balance bronutil.Amount) (bronutil.Amount, uint32) {
...
func (m *mockConstraints) MinChanSize() bronutil.Amount {
	return 1e7
}
func (m *mockConstraints) MaxChanSize() bronutil.Amount {
	return 1e8
}
...
func (m *mockHeuristic) NodeScores(g ChannelGraph, chans []LocalChannel,
	chanSize bronutil.Amount, nodes map[NodeID]struct{}) (
...
func setup(t *testing.T, initialChans []LocalChannel) (*testContext, func()) {
	...
	testCfg := Config{
		...
		Heuristic:      heuristic,
		ChanController: chanController,
		WalletBalance: func() (bronutil.Amount, error) {
			ctx.Lock()
			defer ctx.Unlock()
...
func respondWithScores(t *testing.T, testCtx *testContext,
	channelBudget bronutil.Amount, existingChans, newChans int,
...
func checkChannelOpens(t *testing.T, testCtx *testContext,
	allocation bronutil.Amount, numChans int) []NodeID {

	var nodes []NodeID

	// The agent should attempt to open channels, totaling what we expect.
	var totalAllocation bronutil.Amount
	chanController := testCtx.chanController.(*mockChanController)
	for i := 0; i < numChans; i++ {
		select {
		case openChan := <-chanController.openChanSignals:
...
func TestAgentChannelSizeAllocation(t *testing.T) {
	...
	// channels.
	expectedAllocation := testCtx.constraints.MaxChanSize() * bronutil.Amount(numNewChannels)
	...
	expectedAllocation = testCtx.constraints.MaxChanSize() *
		bronutil.Amount(numNewChannels)
