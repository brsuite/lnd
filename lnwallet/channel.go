...
func (c *commitment) toDiskCommit(ourCommit bool) *channeldb.ChannelCommitment {
	...
	commit := &channeldb.ChannelCommitment{
		...
		CommitFee:       c.fee,
		FeePerKw:        bronutil.Amount(c.feePerKw),
		CommitTx:        c.txn,
		CommitSig:       c.sig,
...
func (lc *LightningChannel) diskCommitToMemCommit(isLocal bool,
	...
	remoteCommitPoint *bronec.PublicKey) (*commitment, error) {
...
type LightningChannel struct {
	...
	LocalFundingKey *bronec.PublicKey
	...
	// channel.
	RemoteFundingKey *bronec.PublicKey
...
func (lc *LightningChannel) logUpdateToPayDesc(logUpdate *channeldb.LogUpdate,
	...
	remoteDustLimit bronutil.Amount) (*PaymentDescriptor, error) {
	...
	case *lnwire.UpdateFee:
		pd = &PaymentDescriptor{
			...
			Amount: lnwire.NewMSatFromSatoshis(
				bronutil.Amount(wireMsg.FeePerKw),
			),
			EntryType:                FeeUpdate,
			addCommitHeightRemote:    commitHeight,
			removeCommitHeightRemote: commitHeight,
...
func NewBreachRetribution(chanState *channeldb.OpenChannel, stateNum uint64,
	...
	commitmentSecret, commitmentPoint := bronec.PrivKeyFromBytes(
...
func HtlcIsDust(chanType channeldb.ChannelType,
	...
	htlcAmt, dustLimit bronutil.Amount) bool {
...
// new commitment.
func (lc *LightningChannel) NextRevocationKey() (*bronec.PublicKey, error) {
	lc.RLock()
	defer lc.RUnlock()
...
// any new states.
func (lc *LightningChannel) InitNextRevocation(revKey *bronec.PublicKey) error {
	lc.Lock()
	defer lc.Unlock()
...
// format. See https://github.com/brsuite/lightning-rfc/issues/728
//
...
func (lc *LightningChannel) getSignedCommitTx() (*wire.MsgTx, error) {
	...
	theirSig, err := bronec.ParseDERSignature(
		localCommit.CommitSig, bronec.S256(),
	)
	if err != nil {
		return nil, err
	}
...
func NewUnilateralCloseSummary(chanState *channeldb.OpenChannel, signer input.Signer,
	...
	remoteCommit channeldb.ChannelCommitment,
	commitPoint *bronec.PublicKey) (*UnilateralCloseSummary, error) {
	...
	closeSummary := channeldb.ChannelCloseSummary{
		...
		SettledBalance:          bronutil.Amount(localBalance),
...
func newOutgoingHtlcResolution(signer input.Signer,
	...
	htlcSig, err := bronec.ParseDERSignature(htlc.Signature, bronec.S256())
	if err != nil {
		return nil, err
	}
...
func (lc *LightningChannel) CreateCloseProposal(proposedFee bronutil.Amount,
	localDeliveryScript []byte,
	remoteDeliveryScript []byte) (input.Signature, *chainhash.Hash,
	bronutil.Amount, error) {
...
func (lc *LightningChannel) CompleteCooperativeClose(
	...
	proposedFee bronutil.Amount) (*wire.MsgTx, bronutil.Amount, error) {
...
func (lc *LightningChannel) UpdateFee(feePerKw chainfee.SatPerKWeight) error {
	...
	pd := &PaymentDescriptor{
		LogIndex:  lc.localUpdateLog.logIndex,
		Amount:    lnwire.NewMSatFromSatoshis(bronutil.Amount(feePerKw)),
		EntryType: FeeUpdate,
	}
...
func CreateCooperativeCloseTx(fundingTxIn wire.TxIn,
	localDust, remoteDust, ourBalance, theirBalance bronutil.Amount,
...
// fee rate (fee-per-kw).
func (lc *LightningChannel) CalcFee(feeRate chainfee.SatPerKWeight) bronutil.Amount {
...
func (lc *LightningChannel) RemoteNextRevocation() *bronec.PublicKey {
	lc.RLock()
	defer lc.RUnlock()
...
// the channel.
func (lc *LightningChannel) MarkDataLoss(commitPoint *bronec.PublicKey) error {
	lc.Lock()
	defer lc.Unlock()
...
func (lc *LightningChannel) LocalChanReserve() bronutil.Amount {
