...
func (i testInputType) output(t *testing.T,
	...
	switch i {
	case plainP2WKH:
		h := bronutil.Hash160(privKey.PubKey().SerializeCompressed())
		addr, err = bronutil.NewAddressWitnessPubKeyHash(h, netParams)
	...
	case nestedP2WKH:
		h := bronutil.Hash160(privKey.PubKey().SerializeCompressed())
		witnessAddr, err := bronutil.NewAddressWitnessPubKeyHash(
			h, netParams,
		)
		require.NoError(t, err)
		...
		addr, err = bronutil.NewAddressScriptHash(
	...
	case singleKeyP2WSH:
		...
		addr, err = bronutil.NewAddressWitnessScriptHash(h[:], netParams)
		require.NoError(t, err)
