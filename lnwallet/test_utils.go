...
import (
	...
	"github.com/brsuite/lnd/shachain"
)
...
var (
	...
	aliceDustLimit = bronutil.Amount(200)
	bobDustLimit   = bronutil.Amount(1300)
...
func CreateTestChannels(chanType channeldb.ChannelType) (
	*LightningChannel, *LightningChannel, func(), error) {
	...
	channelCapacity, err := bronutil.NewAmount(testChannelCapacity)
	if err != nil {
...
func blockFromHex(blockHex string) (*bronutil.Block, error) {
	bytes, err := hex.DecodeString(blockHex)
	if err != nil {
		return nil, err
	}
	return bronutil.NewBlockFromBytes(bytes)
}

// txFromHex parses a full Brocoin transaction from a hex encoded string.
func txFromHex(txHex string) (*bronutil.Tx, error) {
	bytes, err := hex.DecodeString(txHex)
	if err != nil {
		return nil, err
	}
	return bronutil.NewTxFromBytes(bytes)
}
...
func calcStaticFee(chanType channeldb.ChannelType, numHTLCs int) bronutil.Amount {
	const (
		htlcWeight = 172
		feePerKw   = bronutil.Amount(24/4) * 1000
	)
	return feePerKw *
		(bronutil.Amount(CommitWeight(chanType) +
