...
type ErrInsufficientFunds struct {
	amountAvailable bronutil.Amount
	amountSelected  bronutil.Amount
}
...
// change+fees.
func selectInputs(amt bronutil.Amount, coins []Coin) (bronutil.Amount, []Coin, error) {
	satSelected := bronutil.Amount(0)
	for i, coin := range coins {
		satSelected += bronutil.Amount(coin.Value)
		if satSelected >= amt {
...
func calculateFees(utxos []Coin, feeRate chainfee.SatPerKWeight) (bronutil.Amount,
...
func sanityCheckFee(totalOut, fee bronutil.Amount) error {
...
func CoinSelect(feeRate chainfee.SatPerKWeight, amt, dustLimit bronutil.Amount,
	coins []Coin) ([]Coin, bronutil.Amount, error) {
...
func CoinSelectSubtractFees(feeRate chainfee.SatPerKWeight, amt,
	dustLimit bronutil.Amount, coins []Coin) ([]Coin, bronutil.Amount,
	bronutil.Amount, error) {
