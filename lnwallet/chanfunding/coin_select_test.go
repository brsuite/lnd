...
func TestCalculateFees(t *testing.T) {
	...
	type testCase struct {
		...
		expectedFeeNoChange   bronutil.Amount
		expectedFeeWithChange bronutil.Amount
		expectedErr           error
	}
...
func TestCoinSelect(t *testing.T) {
	...
	const dustLimit = bronutil.Amount(1000)

	type testCase struct {
		name        string
		outputValue bronutil.Amount
		coins       []Coin

		expectedInput  []bronutil.Amount
		expectedChange bronutil.Amount
		expectErr      bool
	}
...
func TestCoinSelectSubtractFees(t *testing.T) {
	...
	const dustLimit = bronutil.Amount(1000)
	const dust = bronutil.Amount(100)
	...
	type testCase struct {
		name       string
		highFee    bool
		spendValue bronutil.Amount
		coins      []Coin

		expectedInput      []bronutil.Amount
		expectedFundingAmt bronutil.Amount
		expectedChange     bronutil.Amount
		expectErr          string
	}
