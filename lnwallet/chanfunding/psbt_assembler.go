...
func (i *PsbtIntent) FundingParams() (bronutil.Address, int64, *psbt.Packet,
	error) {
	...
	addr, err := bronutil.NewAddressWitnessScriptHash(
...
// transaction is constructed outside of lnd by using partially signed brocoin
// transactions (PSBT).
type PsbtAssembler struct {
	// fundingAmt is the total amount of coins in the funding output.
	fundingAmt bronutil.Amount
...
// creating a new one.
func NewPsbtAssembler(fundingAmt bronutil.Amount, basePsbt *psbt.Packet,
