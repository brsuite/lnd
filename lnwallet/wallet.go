...
import (
	...
	"github.com/brsuite/lnd/lnwallet/chanvalidate"
...
const (
	...
	// set fee rate.
	anchorChanReservedValue = bronutil.Amount(10_000)
...
func (l *LightningWallet) ConfirmedBalance(confs int32,
	account string) (bronutil.Amount, error) {
...
func (l *LightningWallet) CheckReservedValue(in []wire.OutPoint,
	out []*wire.TxOut, numAnchorChans int) (bronutil.Amount, error) {
	...
	// them.
	var walletBalance bronutil.Amount
	...
	for _, txOut := range out {
		...
		for _, addr := range addrs {
			...
			walletBalance += bronutil.Amount(txOut.Value)
	...
	reserved := bronutil.Amount(numAnchorChans) * anchorChanReservedValue
...
func CreateCommitmentTxns(localBalance, remoteBalance bronutil.Amount,
	...
	otxn := bronutil.NewTx(ourCommitTx)
	...
	ttxn := bronutil.NewTx(theirCommitTx)
