...
import (
	...
	"github.com/brsuite/lnd/chainntnfs/brondnotify"
...
func testDualFundingReservationWorkflow(miner *rpctest.Harness,
	...
	fundingAmount, err := bronutil.NewAmount(5)
	if err != nil {
...
func testListTransactionDetails(miner *rpctest.Harness,
	...
	for _, txDetail := range txDetails {
		...
		if txOuts, ok := blockTxOuts[*txDetail.BlockHash][txDetail.Hash]; !ok {
			...
			var destinationAddresses []bronutil.Address
...
func testSignOutputUsingTweaks(r *rpctest.Harness,
	...
	for i := 0; i < 2; i++ {
		...
		pubkeyHash := bronutil.Hash160(tweakedKey.SerializeCompressed())
		keyAddr, err := bronutil.NewAddressWitnessPubKeyHash(pubkeyHash,
...
func testSingleFunderExternalFundingTx(miner *rpctest.Harness,
	...
	fundingIntent, err := aliceChanFunder.ProvisionChannel(&chanfunding.Request{
		LocalAmt: bronutil.Amount(chanAmt),
		MinConfs: 1,
		FeeRate:  253,
		ChangeAddr: func() (bronutil.Address, error) {
	...
	aliceExternalFunder := chanfunding.NewCannedAssembler(
		thawHeight, *chanPoint, bronutil.Amount(chanAmt), &aliceFundingKey,
		bobFundingKey.PubKey, true,
	)
	bobShimIntent, err := chanfunding.NewCannedAssembler(
		thawHeight, *chanPoint, bronutil.Amount(chanAmt), &bobFundingKey,
