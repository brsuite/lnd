...
func (t *txInputSetState) totalOutput() bronutil.Amount {
...
func (t *txInputSet) addToState(inp input.Input, constraints addConstraints) *txInputSetState {
	...
	if reqOut != nil {
		...
		if bronutil.Amount(reqOut.Value) < dustLimit {
			return nil
		}
	}
	...
	value := bronutil.Amount(inp.SignDesc().Output.Value)
	s.inputTotal += value
	...
	if reqOut != nil {
		s.requiredOutput += bronutil.Amount(reqOut.Value)
	}
